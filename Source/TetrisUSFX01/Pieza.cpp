...
void APieza::setBloques(TMultiMap<FString, std::pair<float, float>>& TipoBloqueYposicion) {
    ...
    ABlock* BloqueX;
    AbloqueRedondo* bloqueChangeColor;
    UMaterialInterface* materialExtra;
    for (auto& Elem : TipoBloqueYposicion)
    {
       ...
       /* UE_LOG(LogTemp, Log, TEXT("Posicion: %d, Tipo de Pieza: %s"), pos, *data[0]);
        ...
        BloqueX = FabricaNivel1a->CrearBloque("color");
        ...
        if (tipoBloque == "rojo") {
            materialExtra = LoadObject<UMaterialInterface>(nullptr, TEXT("Material'/Game/Mesh/Material_6.Material_6'"));
        }
        else if (tipoBloque == "morado") {
            materialExtra = LoadObject<UMaterialInterface>(nullptr, TEXT("Material'/Game/Mesh/Material_5.Material_5'"));
        }
        else {
            materialExtra = LoadObject<UMaterialInterface>(nullptr, TEXT("Material'/Game/Mesh/Material_3.Material_3'"));
        }
        bloqueChangeColor = Cast<AbloqueRedondo>(BloqueX);
        bloqueChangeColor->BlockMesh->SetMaterial(1,materialExtra);
    }
}
